...
def convert_raw_to_iso(input_file, output_iso):
    """Convert a .raw file to .iso format using genisoimage."""
    try:
        command = [
            "genisoimage",
            "-o", output_iso,
        ...
        print("Error: genisoimage is not installed. Please install it and try again.")
        sys.exit(1)
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python rawimg2iso.py <input_file.raw.xz> <output_file.iso>")
        sys.exit(1)
